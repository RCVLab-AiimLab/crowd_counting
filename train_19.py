...
import pathlib
from model_19 import ComputeLoss, CSRNet
from dataset_7 import listDataset
...
# GENERAL
parser.add_argument('--model_desc', default='train_19/', help="Set model description")
parser.add_argument('--pre_model_desc', default='train_7_pre/', help="Set model description")
...
def train(args, model, optimizer, train_list, val_list, train_list_depth, val_list_depth, tb_writer, CUDA):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for bi, (img_big, target_big, img_big_depth, target_big_csr) in pbar:  # batch ----------
            ...
            for i in range(ni):  
                for j in range(nj):  
                    ...
                    img_chip_d = torch.zeros_like(img_chip)
                    img_chip_d[:,0,:,:] = img_chip_depth[:,0,:,:]
                    img_chip_d[:,1,:,:] = img_chip_depth[:,0,:,:]
                    img_chip_d[:,2,:,:] = img_chip_depth[:,0,:,:]
                    # print(img_chip_d.shape)
                    ...
                    target_chip_csr = target_big_csr[:, y1:y2, x1:x2]
                    
                    # print(target_chip.shape)
                    # print(type(target_chip_csr.numpy()))
                    target_chip_csr = zeropad(target_chip_csr.squeeze(0).numpy(), length - target_chip_csr.shape[1], length - target_chip_csr.shape[2], target=True)
                    target_chip_csr = cv2.resize(target_chip_csr,(target_chip_csr.shape[1]//8,target_chip_csr.shape[0]//8),interpolation = cv2.INTER_CUBIC)*64
                    
                    target_chip_csr = torch.from_numpy(target_chip_csr).unsqueeze(0)
                    
                    # print(target_chip_csr.shape)
                    # print(target_chip.shape)

                    assert img_chip.shape[2] == img_chip.shape[3] == length, 'image size error'
                    assert img_chip_d.shape[2] == img_chip_d.shape[3] == length, ' depth image size error'
                    assert target_chip.shape[1] == target_chip.shape[2] == length, 'target size error'
                    assert target_chip_csr.shape[1] == target_chip_csr.shape[2] == length//8, 'target size error'
            
                    if args.vis:
                        vis_input(img_big.squeeze(0), target_big.squeeze(0))
                        vis_input(img_chip.squeeze(0), target_chip.squeeze(0))
                        if args.depth:
                            vis_input(img_big_depth.squeeze(0), img_chip_d.squeeze(0))
                    ...
                    density.append(torch.tensor(target_chip_csr))

                    if args.depth:
                        # img = torch.cat((img_chip ,img_chip_d), dim=1)
                        img = torch.clone(img_chip)
                        img_depth = torch.clone(img_chip_d)
                    else:
                        img = torch.clone(img_chip)
                    ...
                    if i == (ni-1) and j == (nj-1):
                        ...
                        pred0, pred1, pred2, count, density_out = model(imgs, imgs_depth, training=True)  # forward
                        # print('pred0', pred0[0].sum())
                        # print('pred1', pred1[0].sum())
                        # print('pred2', pred2[0].sum())
                        # print('count', count[0])
                        loss, lcount_0, lcount_1, lcount_2 = compute_loss(pred0, pred1, pred2, targets, count, density, density_out) 
...
def validate(args, val_list, val_list_depth, model, CUDA, compute_loss):
    ...
    with open(os.path.join(args.log_dir,'results.txt'), 'w') as f:
        for bi, (img_big, target_big, img_big_depth, target_big_csr) in pbar:  
            ni = int(math.ceil(img_big.shape[2] / length))  
            nj = int(math.ceil(img_big.shape[3] / length)) 
            for i in range(ni):  
                for j in range(nj):  
                    ...
                    if i == (ni-1) and j == (nj-1):
                        ...
                        with torch.no_grad():
                            predictions0, predictions1, predictions2, count, density_out = model(imgs, imgs_depth, training=False)
                            loss, _, _, _ = compute_loss(predictions0, predictions1, predictions2, targets, count, density, density_out)  
