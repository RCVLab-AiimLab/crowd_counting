...
class CSRNet(nn.Module):
    ...
    def forward(self, x, training=True):
        ...
        # x_flat_tot = nn.Linear(x_flat_tot.shape[-1], 1024, device=device)(x_flat_tot)
        # x_flat_tot = nn.ReLU()(x_flat_tot)
        # x_flat_tot = nn.Linear(x_flat_tot.shape[-1], 512, device=device)(x_flat_tot)
        # x_flat_tot = nn.ReLU()(x_flat_tot)
        # x_flat_tot = nn.Linear(x_flat_tot.shape[-1], 128, device=device)(x_flat_tot)
        # x_flat_tot = nn.ReLU()(x_flat_tot)
        # x_flat_tot = nn.Linear(x_flat_tot.shape[-1], 1, device=device)(x_flat_tot)
        ...
        # print('x2', x2_flat.sum())
        # print('x_flat_tot', x_flat_tot.sum())
...
class ComputeLoss:
    ...
    def __call__(self, p0, p1, p2, targets, count): 
        ...
        return lcount_0 + lcount_1 , lcount_0.detach(), lcount_1.detach(), lcount_2.detach()
