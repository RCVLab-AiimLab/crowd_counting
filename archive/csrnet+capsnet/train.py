...
parser.add_argument('--anneal_alpha', default="1", help="Set annealing function for alpha. Options: [none, 1, 2]")
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        best_pred = train(train_list, val_list, model, optimizer, epoch, alpha, best_pred, CUDA)
...
def train(train_list, val_list, model, optimizer, epoch, alpha, best_pred, CUDA):
        ...
        for i in range(ni):  
            for j in range(nj):  
                ...
                if b_num >= 256:
                    ...
                    losses_rec.update(rec_loss.item(), img.size(0))
                    losses_marg.update(marg_loss.item(), img.size(0))
    ...
    tb_writer.add_scalar('train loss/reconstruction', losses_rec.avg, epoch)
    tb_writer.add_scalar('train loss/margin', losses_marg.avg, epoch)
