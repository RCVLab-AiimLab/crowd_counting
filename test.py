...
parser.add_argument('--model_desc', default='shanghaiA_cell__128_128_64__lr10-4_ep50_nodepth_shuffle/', help="Set model description")
parser.add_argument('--dataset_path', default=os.path.join(path,'datasets/shanghai'), help='path to dataset')
...
def test():
    ...
    img_paths = []
    for path_name in path_sets:
        for img_path in glob.glob(os.path.join(path_name, '*.jpg')):
    ...
    for thresh in args.threshold:
        ...
        with open(args.log_dir + 'results_test.txt', 'w') as f:
            for bi, img_path in pbar: 
                ...
                transform_depth = transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean=[0.502], std=[0.291])])
                ...
                if args.depth:
                    ...
                    img_big_depth = F.to_pil_image(img_big_depth)
                    img_big_depth = transform_depth(img_big_depth)
                    print('img_big_depth',img_big_depth.shape)
                ...
                for i in range(ni):  
                    for j in range(nj):  
                        ...
                        if args.depth:
                            ...
                            # print('img_chip_depth', img_chip_depth.shape)
                            img_chip_depth  = zeropad(img_chip_depth.permute(1,2,0).numpy(), length - img_chip_depth.shape[1], length - img_chip_depth.shape[2])
                            img_chip_depth = torch.from_numpy(img_chip_depth).unsqueeze(2).permute(2,0,1)
                            # print('img_chip_depth', img_chip_depth.shape)
                        ...
                        if i == (ni-1) and j == (nj-1):
                            ...
                            with torch.no_grad():
                                ...
                                if args.vis_image:
                                    pred = torch.zeros((imgs.shape[0],imgs.shape[2], imgs.shape[3]))
                                    for i in range(imgs.shape[0]):
                                        pred[i, ...] = predictions0[i].repeat(64,64)
                                    vis_image(args, img_name, img_big, imgs, target_chips, ni, nj, predictions0, predictions0, thresh, bi)
                                ...
                                if args.vis_patch:
                                    ...
                                    for i in range(imgs.shape[0]):
                                        pred[i, ...] = predictions0[i].repeat(64,64)
                                    vis_input(imgs[0, ...], target_chips[0, ...],  bi, path,'chip', args, predicted=pred[0, :, :])
...
def vis_image(args, img_name, img_big, imgs, target_chips, ni, nj, predictions0, predictions1, thresh, bi):
    ...
    dir = os.path.join(path,'images','chip',args.model_desc,'im_'+str(bi)+'.jpg')
