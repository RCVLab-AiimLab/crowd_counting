...
def parse_model(d):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv]:
            ...
            c1, c2 = ch[f], args[0] #c1 = 3, c2 = 16
            # print(c1)
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]] #[3,16,1,1]
            # print('args is ',args)
...
class Detect(nn.Module):
    ...
    def forward(self, x, inference):
        ...
        x = self.m[0](x[0])  # conv Conv2d(256, 2, kernel_size=(1, 1), stride=(1, 1))
...
class ComputeLoss:
    ...
    def __call__(self, p, targets): 
        # print('p shape is',p.shape)#[512, 2, 16, 16]
        # print('target shape is',targets.shape)#[187, 3]
        device = targets.device
        lobj = torch.zeros(1, device=device)
        lnum = torch.zeros(1, device=device)
        # print('lobj is',lobj)
        # print('lnum is',lnum)
        indices = self.build_targets(p, targets)  #generate indices of head locations
